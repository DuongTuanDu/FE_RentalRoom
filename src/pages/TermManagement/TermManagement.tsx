...
const TermManagement = () => {
  ...
  const { data: termsData, isLoading: isTermsLoading, isFetching: isTermsFetching } = useGetTermsQuery(
    {
  ...
  const handleSubmit = async (data: { name: string; description: string; status?: "active" | "inactive" }) => {
    ...
    try {
      if (editingTerm) {
        await updateTerm({ id: editingTerm._id, data: { name, description, status: status || "active" } }).unwrap();
        toast.success("Cập nhật điều khoản thành công");
      } else {
        await createTerm({ buildingId: selectedBuildingId, name, description }).unwrap();
    ...
    <div className="container mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h1 className="text-3xl font-bold tracking-tight">Quản lý điều khoản</h1>
          <p className="text-muted-foreground">Tạo và quản lý điều khoản cho từng tòa nhà</p>
        </div>
        <Button onClick={handleOpenCreateModal} className="gap-2" disabled={!selectedBuildingId}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Chọn tòa nhà và trạng thái để xem điều khoản</CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v: "all" | "active" | "inactive") => setSelectedStatus(v)}
            >
              <SelectTrigger>
        ...
        <CardContent>
              ...
              <TableBody>
                {!selectedBuildingId ? (
                  ...
                  termsData.data.map((term, idx) => {
                    const plainDescription = stripHtmlTags(term.description || "");
                    return (
                      <TableRow key={term._id}>
                        <TableCell>{(currentPage - 1) * pageLimit + idx + 1}</TableCell>
                        ...
                        <Badge variant={term.status === "active" ? "default" : "secondary"}>
                          {term.status === "active" ? "Hoạt động" : "Ngừng hoạt động"}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Button variant="outline" size="icon" onClick={() => handleOpenEditModal(term)}>
          ...
          <div className="flex items-center justify-between pt-4">
            <p className="text-sm text-muted-foreground">
              Hiển thị <span className="font-medium">{totalItems === 0 ? 0 : (currentPage - 1) * pageLimit + 1}</span> đến
              <span className="font-medium"> {Math.min(currentPage * pageLimit, totalItems)}</span> trong tổng số
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => setCurrentPage((p) => (totalPages ? Math.min(totalPages, p + 1) : p))}
                disabled={totalPages === 0 || currentPage >= totalPages || isTermsFetching}
              >
                Trang sau
              </Button>
            </div>
